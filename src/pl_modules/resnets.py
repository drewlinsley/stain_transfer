...
def resunet_restaining_color_he_input(in_channel=3, out_channel=2):
    # assert num_classes is not None, "You must pass the number of classes to your model."
    model = ResUnet_restaining(in_channel=in_channel, out_channel=out_channel)  # , num_classes=num_classes)
    # model.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
    # model.maxpool = nn.Identity()
    return model


def resunet_restaining_polyt_dapi_input(in_channel=2, out_channel=3):
    # assert num_classes is not None, "You must pass the number of classes to your model."
    model = ResUnet_restaining(in_channel=in_channel, out_channel=out_channel)  # , num_classes=num_classes)
    # model.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
    # model.maxpool = nn.Identity()
...
def resunet_restaining_seqfish_input(in_channel=2, out_channel=43):
    # assert num_classes is not None, "You must pass the number of classes to your model."
    model = ResUnet_seqfish(in_channel=in_channel, out_channel=out_channel)  # , num_classes=num_classes)
    # model.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
    # model.maxpool = nn.Identity()
    return model
...
class ResUnet_seqfish(nn.Module):
    def __init__(self, in_channel=1, out_channel=1, default_channels=2, filters=[64, 128, 256, 384], control=False):
        super(ResUnet_seqfish, self).__init__()
        ...
        for idx in range(out_channel):
            self.output_layers.append(nn.Conv2d(filters[0], self.default_channels, 1, 1))
    ...
    def forward(self, x):
        ...
        # Return outputs
        re_output = torch.stack((outputs), 1)
        # re_output = output_0.reshape(-1, self.out_channel, self.default_channels, x.shape[-2], x.shape[-1])
        return ad_pred, re_output


class ResUnet_restaining(nn.Module):
    def __init__(self, in_channel=1, out_channel=1, default_channels=256, filters=[64, 128, 256, 384], control=False):
        ...
        self.output_layer_1 = nn.Conv2d(filters[0], self.default_channels, 1, 1)
        self.output_layer_2 = nn.Conv2d(filters[0], self.default_channels, 1, 1)
    ...
    def forward(self, x):
        ...
        output_1 = self.output_layer_1(x10_0)
        output_2 = self.output_layer_2(x10_0)
        # output_0 = torch.sigmoid(output_0)
        ...
        # Return outputs
        re_output = torch.stack((output_0, output_1, output_2), 1)
        # re_output = output_0.reshape(-1, self.out_channel, self.default_channels, x.shape[-2], x.shape[-1])
        return ad_pred, re_output
